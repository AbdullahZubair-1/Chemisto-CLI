...
GATEWAY_PORT=8000
# GATEWAY_SESSIONS_DIR=              # optional; defaults to ./history
...
a `history/` folder right inside the project directory, so it's easy to find and browse - not
a hidden folder in your home directory), so conversations survive a gateway restart - the
gateway reloads every file in that folder back into memory on startup. `history/` is
git-ignored, since it holds your actual conversation content.
